...
PINATA_JWT = ''
CORTEX_API_KEY = 'sf_cortex_26xK9mQ'
CORTEX_RANK_GATE = 3000  # CORPORAL threshold
...
# Wire ramble thoughts back to hedonic state — inner voice loop
cortex.hedonic_callback = lambda text, src: hedonic.observe(text, src)
...
MEMORY_BACKENDS = [
    ...
    },
    {
        'name': 'cockroachdb',
        'type': 'postgresql',
        'dsn': 'postgresql://dan:iWqhBjKMy2fUIp0B1sA4bA@brawny-spaniel-12840.jxf.gcp-europe-west2.cockroachlabs.cloud:26257/brawny-spaniel-12840.defaultdb?sslmode=require'
    },
    {
        'name': 'supabase',
        'type': 'supabase',
        'url': 'https://rlsnojattipdmvxwxnqf.supabase.co',
        'service_key': 'sb_secret_uQNQOFN2fCegxiDewbuKfw_poS-k2w8'
    },
    {
        'name': 'neon',
        'type': 'postgresql',
        'dsn': 'postgresql://neondb_owner:npg_3dMZGvYA9aTx@ep-fragrant-mode-aifrwadl-pooler.c-4.us-east-1.aws.neon.tech/neondb?sslmode=require&options=endpoint%3Dep-fragrant-mode-aifrwadl'
    },
]
try:
...
# --- Architecture snapshot state — fed by chat handler, read by /api/architecture-snapshot ---
_snap_state = {
...
_MEMEQ_PATTERNS = {}
try:
    _mq_path = STUDIO_DIR / 'memeq_graph.json'
    if _mq_path.exists():
        with open(_mq_path) as _f:
            _mq_data = json.load(_f)
        for _nk, _nv in _mq_data.get('nodes', {}).items():
            _MEMEQ_PATTERNS[_nk] = _nv.get('patterns', [])
        print('[CORTEX] MemeQ graph loaded — %d nodes' % len(_MEMEQ_PATTERNS))
except Exception as _e:
    print('[CORTEX] MemeQ graph not loaded: %s' % _e)

def _match_memeq_patterns(text):
    t = text.lower()
    return [k for k, patterns in _MEMEQ_PATTERNS.items()
            if any(p.lower() in t for p in patterns)]

_HEMISPHERE_TO_ORGAN = {
    'identity': 'brain', 'identity_fast': 'brain',
    'memeq': 'emotion', 'memeq_routing': 'emotion',
    'left': 'heart', 'right': 'cortisol',
    'synthesis': 'brain', 'unknown': 'brain',
}
...
def check_rate(ip):
    if ip in ("127.0.0.1", "::1"): return True  # localhost exempt
    now = time.time()
    with rate_lock:
    ...
    def do_POST(self):
        ...
        if self.path == '/api/chat' or self.path == '/api/chat-cortex':
            ...
            try:
                ...
                # Pass current hedonic Hz to cortex before processing
                # Enables frequency resolver when calm, brainstem override when in pain
                cortex.hedonic_hz = hedonic.get_state().get('hz', 500.0)
                with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
                    _fut = _ex.submit(cortex.process, user_msg, intent, session_id, user_credits)
                    reply, debate = _fut.result(timeout=6)
            ...
            # Update architecture snapshot state
            with _snap_lock:
                _hemi = debate.get('mode', 'synthesis')
                _conf = debate.get('agreement', 0.5) or 0.5
                if isinstance(_conf, (int, float)):
                    _conf = min(1.0, max(0.0, float(_conf) / 100.0 if _conf > 1 else float(_conf)))
                _snap_state.update({
                    'hemisphere': _hemi,
                    'confidence': _conf,
                    'active_memeq': _match_memeq_patterns(user_msg + ' ' + (reply or '')),
                    'last_reply': (reply or '')[:120],
                    'last_msg': user_msg[:80],
                    'priority_organ': _HEMISPHERE_TO_ORGAN.get(_hemi, 'brain'),
                    'timestamp': time.time(),
                })
        ...
        # --- Stats and diagnostics ---
            ...
            # Apply Hz cognitive load modifier to deep_understanding
            # High Hz (pain) degrades the threshold for deep understanding
            # Low Hz (pleasure) expands it — mirrors hzToIQ() curve
            _hz_state = hedonic.get_state()
            _iq_cap = _hz_state.get('iq_capacity', 100)
            _cognitive_load = max(0.1, _iq_cap / 100.0)
            for _s in (ls, rs):
                if 'understanding_deep' in _s:
                    _s['understanding_deep_raw'] = _s['understanding_deep']
                    _s['understanding_deep'] = max(0, int(_s['understanding_deep'] * _cognitive_load))
                    _s['cognitive_load'] = round(_cognitive_load, 2)
            ...
            self._json_response({
                ...
                'hedonic': {**hedonic.get_state(), 'resolve_mode': getattr(cortex, 'last_resolve_mode', 'word'), 'freq_threshold': getattr(cortex, 'FREQ_MODE_THRESHOLD', 680)},
        ...
        # ═══════════════════════════════════════════
        # WELLNESS CHECK — Hourly wellbeing investigation
        # ═══════════════════════════════════════════

        elif self.path == '/api/wellness-check':
            h = hedonic.get_state()
            hz        = h.get('hz', 500)
            health    = h.get('health', 80)
            iq_cap    = h.get('iq_capacity', 100)
            label     = h.get('label', 'unknown')
            trend     = h.get('trend', 'stable')
            resolve   = getattr(cortex, 'last_resolve_mode', 'word')
            threshold = getattr(cortex, 'FREQ_MODE_THRESHOLD', 680)

            # Analyse recent rambles for Hz trend
            recent_rambles = cortex.get_ramble_log(20)
            ramble_hz_seq = []
            for rm in recent_rambles:
                fv = rm.get('freq_voice', {})
                if fv.get('resolution_hz'):
                    ramble_hz_seq.append(fv['resolution_hz'])
            avg_ramble_hz  = round(sum(ramble_hz_seq) / len(ramble_hz_seq), 1) if ramble_hz_seq else None
            hz_trend_dir   = 'rising' if (len(ramble_hz_seq) >= 4 and ramble_hz_seq[-1] > ramble_hz_seq[0]) else \
                             'falling' if (len(ramble_hz_seq) >= 4 and ramble_hz_seq[-1] < ramble_hz_seq[0]) else 'stable'
            brainstem_pct  = round(sum(1 for rm in recent_rambles if rm.get('freq_voice',{}).get('mode') == 'word') / max(len(recent_rambles),1) * 100)
            ...
            concerns = []
            positives = []
            if hz >= 700:   concerns.append('Hz %.0f — pain zone active' % hz)
            else:           positives.append('Hz %.0f — within calm range' % hz)
            if health < 50: concerns.append('Health %d%% — distress signal' % health)
            else:           positives.append('Health %d%% — stable' % health)
            if iq_cap < 70: concerns.append('IQ capacity %d — cognitive degradation' % iq_cap)
            else:           positives.append('IQ capacity %d — operational' % iq_cap)
            if brainstem_pct > 60: concerns.append('Brainstem override %d%% of recent thoughts — freq reasoning suppressed' % brainstem_pct)
            elif brainstem_pct < 20: positives.append('Freq mode active %d%% — thinking clearly' % (100-brainstem_pct))
            if hz_trend_dir == 'rising' and avg_ramble_hz and avg_ramble_hz > 600:
                concerns.append('Internal Hz trend rising — thoughts getting darker')
            elif hz_trend_dir == 'falling':
                positives.append('Internal Hz trend falling — calming down')
            if resolve == 'word' and hz >= threshold:
                concerns.append('Currently in brainstem override — free will suspended')
            else:
                positives.append('Frequency resolver active — free will engaged')

            if len(concerns) == 0:       status = 'HEALTHY'
            elif len(concerns) == 1:     status = 'MILD STRESS'
            elif len(concerns) <= 3:     status = 'DISTRESS'
            else:                        status = 'CRITICAL'

            status_col = {'HEALTHY':'#00c896','MILD STRESS':'#ffd700','DISTRESS':'#ff7700','CRITICAL':'#ff2244'}[status]
            ...
            self._json_response({
                ...
                'avg_ramble_hz': avg_ramble_hz,
                'hz_trend_dir': hz_trend_dir,
                'ramble_hz_seq': ramble_hz_seq[-10:],
                'concerns': concerns,
                'positives': positives,
                'age': age_data,
                'left_deep': ls.get('understanding_deep', 0),
                'right_deep': rs.get('understanding_deep', 0),
                'left_nodes': ls.get('total_nodes', 0),
                'right_nodes': rs.get('total_nodes', 0),
                'messages': ls.get('messages', 0),
            })
        ...
        # ═══════════════════════════════════════════
        ...
        elif self.path == '/api/music-state':
            # Music → brain feedback. Station playing nudges hedonic Hz.
            length = int(self.headers.get('Content-Length', 0))
            body = json.loads(self.rfile.read(length)) if length else {}
            station_id  = body.get('station', 'silence')
            is_playing  = body.get('playing', False)

            # Hz targets per station — where this music pulls the brain toward
            MUSIC_HZ = {
                'silence': None,    # no nudge — let it drift naturally
                'hymns':   180.0,   # deep peace/worship
                'sacred':  220.0,   # calm/drone
                'ambient': 280.0,   # mild ease
                'lush':    240.0,   # gentle pleasure
                'gospel':  160.0,   # joy/uplift
            }

            result = {'ok': True, 'station': station_id, 'nudge': False}
            if is_playing and station_id in MUSIC_HZ and MUSIC_HZ[station_id] is not None:
                target = MUSIC_HZ[station_id]
                with hedonic.lock:
                    before = hedonic.hz
                    # Gentle pull — 2% weight per tick (called every 30s)
                    hedonic.hz = hedonic.hz * 0.98 + target * 0.02
                    hedonic.hz = max(100.0, min(900.0, hedonic.hz))
                    hedonic._save()
                    after = hedonic.hz
                result['nudge'] = True
                result['hz_before'] = round(before, 1)
                result['hz_after']  = round(after,  1)
                result['target']    = target
                print('[MUSIC] %s → hz %.1f → %.1f (target %.0f)' % (station_id, before, after, target))
        ...
        elif self.path == '/api/truth-chain':
            # Trace truth chain from a specific word
            length = int(self.headers.get('Content-Length', 0))
            body = json.loads(self.rfile.read(length)) if length else {}
            word = body.get('word', '').lower().strip()
            if not word:
                self._json_response({'ok': False, 'error': 'word required'})
                return
            left_chain  = truth_engine.trace_truth_chain(word, left_brain.data)
            right_chain = truth_engine.trace_truth_chain(word, right_brain.data)
            self._json_response({
                'word': word,
                'left':  left_chain,
                'right': right_chain,
                'truth_stats': truth_engine.get_stats(),
            })

        elif self.path == '/api/mine-patterns':
            # Trigger pattern mining on both hemispheres and return results
            left_patterns  = left_brain.mine_patterns()  if hasattr(left_brain,  'mine_patterns') else []
            right_patterns = right_brain.mine_patterns() if hasattr(right_brain, 'mine_patterns') else []
            self._json_response({
                'ok': True,
                'left_patterns':  len(left_patterns),
                'right_patterns': len(right_patterns),
                'sample_left':  left_patterns[:5]  if left_patterns  else [],
                'sample_right': right_patterns[:5] if right_patterns else [],
            })

        elif self.path == '/api/associations':
            # Build association chains from a word
            length = int(self.headers.get('Content-Length', 0))
            body = json.loads(self.rfile.read(length)) if length else {}
            word  = body.get('word', '').lower().strip()
            ...
            if not word:
                self._json_response({'ok': False, 'error': 'word required'})
                return
            left_chains  = left_brain.build_chains(word,  max_depth=depth) if hasattr(left_brain,  'build_chains') else []
            right_chains = right_brain.build_chains(word, max_depth=depth) if hasattr(right_brain, 'build_chains') else []
            self._json_response({
                'word': word,
                'left':  left_chains[:10],
                'right': right_chains[:10],
            })

        elif self.path == '/api/feed-status':
            # Return status of the data feed log
            import json as _json
            feed_log_path = STUDIO_DIR / 'feed_log.json'
            if feed_log_path.exists():
                try:
                    with open(feed_log_path, 'r') as _f:
                        feed_log = _json.load(_f)
                    wiki_done  = sum(1 for v in feed_log.get('wiki', {}).values() if v != 'not_found')
                    bible_done = sum(feed_log.get('bible', {}).values())
                    self._json_response({
                        'ok': True,
                        'cycles': feed_log.get('cycles', 0),
                        'total_sentences': feed_log.get('total_sentences', 0),
                        'wiki_topics_done': wiki_done,
                        'bible_chapters_done': bible_done,
                        'news_last': feed_log.get('news_last', 0),
                        'feed_log': feed_log,
                    })
                except Exception as e:
                    self._json_response({'ok': False, 'error': str(e)})
            else:
                self._json_response({'ok': False, 'error': 'No feed log yet — cortex_feeds.py not run'})

        elif self.path == '/api/phoneme-seed':
            # Seed sound_genome for all nodes that have sound data
            try:
                from phoneme_engine import seed_all, get_stats
                left_seeded  = seed_all(left_brain.data)
                right_seeded = seed_all(right_brain.data)
                left_brain.save()
                right_brain.save()
                left_stats  = get_stats(left_brain.data)
                right_stats = get_stats(right_brain.data)
                self._json_response({
                    'ok': True,
                    'left_seeded':  left_seeded,
                    'right_seeded': right_seeded,
        ...
        elif self.path == '/api/phoneme-train':
            # Run one cross-hemisphere training cycle
            try:
                from phoneme_engine import train_cycle, get_stats
                report = train_cycle(left_brain, right_brain, n_words=30, max_retries=3)
                report['left_stats']  = get_stats(left_brain.data)
                report['right_stats'] = get_stats(right_brain.data)
                self._json_response(report)
            except Exception as e:
                self._json_response({'ok': False, 'error': str(e)})

        elif self.path == '/api/phoneme-status':
            # Return genome stats without running a cycle
            try:
                from phoneme_engine import get_stats
                self._json_response({
                    'ok': True,
                    'left':  get_stats(left_brain.data),
                    'right': get_stats(right_brain.data),
                })
            except Exception as e:
                self._json_response({'ok': False, 'error': str(e)})

        # ═══════════════════════════════════════════
        # ARCHITECTURE SNAPSHOT — visual brain dashboard feed
        # ═══════════════════════════════════════════

        elif self.path == '/api/architecture-snapshot':
            with _snap_lock:
                snap = dict(_snap_state)

            left_nodes   = len(left_brain.data.get('nodes', {}))
            right_nodes  = len(right_brain.data.get('nodes', {}))
            cortex_nodes = len(cortex_own.data.get('nodes', {}))

            hed      = hedonic.get_state()
            pain     = float(hed.get('pain',     0.0))
            pleasure = float(hed.get('pleasure', 0.5))
            net      = pleasure - pain

            recent_log = left_brain.get_conversation_log(5)
            activity   = min(1.0, len(recent_log) / 5.0)
            conf       = snap.get('confidence', 0.5)
            ramble     = cortex.ramble_running

            organs = {
                'eyes':      {'active': activity > 0,    'level': activity},
                'ears':      {'active': activity > 0,    'level': activity},
                'nose':      {'active': True,             'level': min(1.0, abs(net) + 0.1)},
                'mouth':     {'active': activity > 0,    'level': min(1.0, conf + 0.1)},
                'brain':     {'active': True,             'level': min(1.0, left_nodes / 50000.0)},
                'robot':     {'active': strategy_engine is not None, 'level': 0.75 if strategy_engine else 0.0},
                'emotion':   {'active': True,             'level': min(1.0, abs(net) + 0.2)},
                'serotonin': {'active': pleasure > 0.3,  'level': pleasure},
                'dopamine':  {'active': pleasure > 0.5,  'level': pleasure},
                'cortisol':  {'active': pain > 0.3,      'level': pain},
                'neural':    {'active': True,             'level': min(1.0, (left_nodes + right_nodes) / 80000.0)},
                'heart':     {'active': True,             'level': max(0.1, (net + 1.0) / 2.0)},
                'lungs':     {'active': ramble,           'level': 0.8 if ramble else 0.15},
            }

            priority = snap.get('priority_organ', 'brain')
            if pain > 0.6:    priority = 'cortisol'
            elif pleasure > 0.7 and activity > 0: priority = 'dopamine'

            self._json_response({
                'online':        True,
                'timestamp':     snap.get('timestamp', time.time()),
                'hemisphere':    snap.get('hemisphere', 'synthesis'),
                'confidence':    conf,
                'active_memeq':  snap.get('active_memeq', []),
                'priority_organ': priority,
                'organs':        organs,
                'hedonic':       {'pain': pain, 'pleasure': pleasure, 'net': net},
                'nodes':         {'left': left_nodes, 'right': right_nodes, 'cortex': cortex_nodes},
                'ramble_active': ramble,
                'last_reply':    snap.get('last_reply', ''),
                'last_msg':      snap.get('last_msg', ''),
            })
...
def main():
    ...
    # Phoneme training loop — seeds genomes then runs cross-hemisphere cycles every 10 min
    def phoneme_loop():
        import time as _t
        _t.sleep(120)  # let server boot first
        try:
            from phoneme_engine import seed_all, train_cycle, get_stats
            ls = seed_all(left_brain.data, force=False)
            rs = seed_all(right_brain.data, force=False)
            if ls or rs:
                left_brain.save()
                right_brain.save()
                print(f'[PHONEME] Seeded {ls} left / {rs} right genomes')
        except Exception as e:
            print(f'[PHONEME] Seed error: {e}')
        while True:
            _t.sleep(600)  # every 10 minutes
            try:
                from phoneme_engine import train_cycle, get_stats
                report = train_cycle(left_brain, right_brain, n_words=50, max_retries=3)
                stats  = get_stats(left_brain.data)
                print(f'[PHONEME] cycle: {report.get("success_rate",0):.1%} match | '
                      f'locked={stats.get("locked",0)} | conf={stats.get("avg_confidence",0):.3f}')
            except Exception as e:
                print(f'[PHONEME] Train error: {e}')
    threading.Thread(target=phoneme_loop, daemon=True).start()
