...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Setup black ops buttons - now triggers pipe bomb sequence first!
...
function updateDisplay() {
  ...
  if (G.trumpHP <= 20 && !G.nukeUsed) {
    ...
    tierEmoji = '☸️';
  } else if (G.trumpHP <= 50 && !G.blackopsUsed) {
    activeTier = 1;
...
function deepStateTurn() {
  ...
  G.deepStateHP += (5 * G.difficultyMult) / scale;
...
function checkGameEnd() {
  ...
  if (G.purity >= 96) {
    ...
    // Add grey death screen overlay with game over video
    addDeathOverlay();
    // Video will handle restart when it ends (no ending screen needed)
  }
}
...
function startFinale() {
  ...
  const qualifies = purity >= 97 && wealth >= 50000 && !hasRecorded && (typeof showNameEntry === 'function');
...
function addDeathOverlay() {
  ...
  // When video ends, restart game
  video.onended = () => {
    console.log('Game over video ended, restarting...');
    setTimeout(() => {
  ...
  // Fallback if video fails to load - restart after 5 seconds
  video.onerror = () => {
...
function showStatsAreWrongVideo() {
  ...
  // After video ends, remove overlay and show ending screen
  video.onended = () => {
    overlay.remove();
    showEndingScreen(determineEnding());
  };
...
function showEndingScreen(type) {
  ...
  const params = new URLSearchParams({
    ...
    ngPlusLoop: G.ngPlusLoop || 0,
    advisor: G.advisor || 1,
    trumpHP: Math.round(G.trumpHP),
    deepStateHP: Math.round(G.deepStateHP)
  });
...
const GAME_HINTS = [
  ...
  { id: 'blackops', text: '★ BLACK OPS AVAILABLE ★', sub: 'When HP drops to 50%, special options appear!', condition: () => G.trumpHP <= 55 && G.trumpHP > 45 && !hintsShown.blackops },
...
function showFeedbackModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:200;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;font-family:"Press Start 2P",monospace;';
  modal.onclick = (e) => { if (e.target === modal) closeFeedbackModal(); };

  modal.innerHTML = `
    <div style="background:#1a1a2e;border:3px solid #f0f;border-radius:12px;padding:20px;max-width:340px;width:90%;box-shadow:0 0 30px rgba(255,0,255,0.4);">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;">
        <div style="color:#f0f;font-size:12px;text-shadow:0 0 10px #f0f;">FEEDBACK</div>
        <div onclick="closeFeedbackModal()" style="color:#f00;font-size:14px;cursor:pointer;padding:5px;">X</div>
      </div>
      <div id="fb-cats" style="display:flex;gap:6px;margin-bottom:12px;">
        <button onclick="setFeedbackCat('bug',this)" class="fb-cat active" style="flex:1;padding:6px 4px;font-family:inherit;font-size:8px;background:#f0f;color:#000;border:2px solid #f0f;border-radius:6px;cursor:pointer;">BUG</button>
        <button onclick="setFeedbackCat('suggestion',this)" class="fb-cat" style="flex:1;padding:6px 4px;font-family:inherit;font-size:8px;background:transparent;color:#f0f;border:2px solid #f0f;border-radius:6px;cursor:pointer;">SUGGEST</button>
        <button onclick="setFeedbackCat('other',this)" class="fb-cat" style="flex:1;padding:6px 4px;font-family:inherit;font-size:8px;background:transparent;color:#f0f;border:2px solid #f0f;border-radius:6px;cursor:pointer;">OTHER</button>
      </div>
      <textarea id="fb-msg" placeholder="What happened?" style="width:100%;height:80px;background:#0a0a1a;color:#fff;border:2px solid #333;border-radius:6px;padding:8px;font-family:inherit;font-size:9px;resize:none;box-sizing:border-box;"></textarea>
      <button onclick="submitFeedback()" style="width:100%;margin-top:10px;padding:10px;font-family:inherit;font-size:10px;background:linear-gradient(135deg,#f0f,#808);color:#fff;border:2px solid #f0f;border-radius:6px;cursor:pointer;text-shadow:1px 1px 0 #000;">SEND</button>
    </div>
  `;
...
function setFeedbackCat(cat, btn) {
  feedbackCategory = cat;
  const cats = document.querySelectorAll('#fb-cats .fb-cat');
  cats.forEach(b => { b.style.background = 'transparent'; b.style.color = '#f0f'; });
...
function submitFeedback() {
  ...
  const gameState = {
    ...
    purity: typeof G !== 'undefined' ? G.purity : null,
    trumpHP: typeof G !== 'undefined' ? G.trumpHP : null,
    deepStateHP: typeof G !== 'undefined' ? G.deepStateHP : null,
    ngPlusLoop: typeof G !== 'undefined' ? G.ngPlusLoop : null,
    advisor: typeof G !== 'undefined' ? G.advisor : null,
    term: typeof G !== 'undefined' ? G.term : null
  };

  fetch('/trump/feedback.php', {
    method: 'POST',
  ...
  .then(() => {
    closeFeedbackModal();
    if (typeof showNews === 'function') showNews('FEEDBACK SENT!');
  })
  .catch(() => {
    closeFeedbackModal();
    if (typeof showNews === 'function') showNews('SEND FAILED - TRY AGAIN');
  });
}
