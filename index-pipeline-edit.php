...
/* PIPELINE PILL + MODAL */
#pipeline-pill{position:fixed;bottom:54px;left:20px;z-index:25;cursor:pointer;display:flex;align-items:center;gap:7px;padding:8px 16px;background:rgba(255,0,64,0.07);border:1px solid rgba(255,0,64,0.28);border-radius:8px;color:#ff3cac;font-size:10px;letter-spacing:2px;font-weight:700;text-transform:uppercase;transition:all 0.25s;pointer-events:all;animation:pillGlow 3s ease-in-out infinite;}
#pipeline-pill:hover{background:rgba(255,0,64,0.14);border-color:rgba(255,0,64,0.55);box-shadow:0 0 20px rgba(255,60,172,0.25);}
@keyframes pillGlow{0%,100%{box-shadow:0 0 0 0 rgba(255,60,172,0);}50%{box-shadow:0 0 14px rgba(255,60,172,0.2);}}
.pill-dot{width:6px;height:6px;border-radius:50%;background:#ff3cac;animation:pillPulse 1.4s ease-in-out infinite;}
@keyframes pillPulse{0%,100%{transform:scale(1);opacity:1;}50%{transform:scale(1.6);opacity:0.5;}}
#pipeline-modal{position:fixed;inset:0;background:rgba(5,5,8,0.97);z-index:180;display:none;align-items:center;justify-content:center;padding:20px;overflow-y:auto;}
#pipeline-modal.open{display:flex;}
#pipeline-close{position:fixed;top:16px;right:20px;background:transparent;border:1px solid rgba(255,255,255,0.1);border-radius:6px;color:#475569;font-size:11px;letter-spacing:2px;padding:6px 14px;cursor:pointer;z-index:190;}
#pipeline-close:hover{color:#fff;border-color:rgba(255,255,255,0.3);}
.pl-phone{width:min(360px,92vw);background:linear-gradient(155deg,#0d0414 0%,#060212 50%,#0a0618 100%);border-radius:44px;border:2.5px solid #2a1a3a;overflow:hidden;box-shadow:0 0 80px rgba(255,0,64,0.18),0 0 160px rgba(138,43,226,0.1);position:relative;}
.pl-island{position:absolute;top:10px;left:50%;transform:translateX(-50%);width:100px;height:28px;background:#000;border-radius:16px;z-index:10;}
.pl-inner{padding:52px 24px 32px;display:flex;flex-direction:column;}
.pl-eyebrow{font-size:8px;letter-spacing:4px;text-transform:uppercase;color:#ff3cac;margin-bottom:6px;text-align:center;}
.pl-brand{font-size:22px;font-weight:900;letter-spacing:3px;color:#fff;text-align:center;margin-bottom:2px;}
.pl-sub{font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#6644aa;text-align:center;margin-bottom:20px;}
.pl-pct-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;}
.pl-pct-label{font-size:9px;letter-spacing:2px;color:#475569;text-transform:uppercase;}
.pl-pct-val{font-size:13px;font-weight:900;color:#ff3cac;letter-spacing:1px;}
.pl-bar{height:6px;background:rgba(255,255,255,0.06);border-radius:3px;overflow:visible;margin-bottom:18px;position:relative;}
.pl-bar-fill{height:100%;width:95%;background:linear-gradient(90deg,#ff0040,#8a2be2,#cc44ff);border-radius:3px;}
.pl-bar-gap{position:absolute;right:0;top:0;width:5%;height:6px;background:rgba(255,60,172,0.25);border:1px dashed #ff3cac;border-radius:2px;}
.pl-check{display:flex;align-items:center;gap:10px;margin-bottom:9px;}
.pl-tick{width:18px;height:18px;border-radius:50%;background:linear-gradient(135deg,#ff0040,#8a2be2);display:flex;align-items:center;justify-content:center;font-size:9px;flex-shrink:0;color:#fff;}
.pl-check-text{font-size:11px;color:#94a3b8;letter-spacing:0.5px;line-height:1.4;}
.pl-check-text b{color:#e2e8f0;}
.pl-call{background:linear-gradient(135deg,rgba(255,0,64,0.1),rgba(138,43,226,0.1));border:1px solid rgba(255,0,64,0.3);border-radius:12px;padding:14px;margin:16px 0 18px;text-align:center;}
.pl-call-title{font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#ff3cac;margin-bottom:7px;}
.pl-call-body{font-size:11px;color:#94a3b8;line-height:1.75;}
.pl-call-body b{color:#fff;}
.pl-cta-main{display:block;text-align:center;padding:13px;background:linear-gradient(90deg,#ff0040,#8a2be2);color:#fff;text-decoration:none;border-radius:10px;font-size:10px;letter-spacing:3px;text-transform:uppercase;font-weight:700;margin-bottom:9px;transition:opacity 0.2s;}
.pl-cta-main:hover{opacity:0.85;}
.pl-cta-sec{display:block;text-align:center;padding:10px;background:transparent;border:1px solid rgba(138,43,226,0.4);color:#aa77ff;text-decoration:none;border-radius:10px;font-size:9px;letter-spacing:3px;text-transform:uppercase;transition:all 0.2s;}
.pl-cta-sec:hover{background:rgba(138,43,226,0.1);border-color:rgba(138,43,226,0.7);}
.pl-quote{text-align:center;font-size:9px;color:#2a3a50;letter-spacing:1px;font-style:italic;margin-top:14px;line-height:1.7;padding:0 8px;}
</style>
</head>
<body>
...
if($_jar_count > 0):
?>
  <div style="margin:12px 0 4px;font-family:'Orbitron',monospace;font-size:7px;color:rgba(200,168,75,0.45);letter-spacing:2px;text-transform:uppercase;"><?php echo $_jar_count; ?> user<?php echo $_jar_count!==1?'s':''; ?> · low rez · initialised in code · consciousness absent</div>
  <div style="font-size:16px;letter-spacing:2px;opacity:0.55;margin-bottom:12px;"><?php echo str_repeat('🫙', min($_jar_count, 50)); ?><?php if($_jar_count>50) echo '<span style="font-size:10px;color:rgba(200,168,75,0.4)"> +'.($_jar_count-50).'</span>'; ?></div>
<?php endif; ?>
  <button id="intro-begin">BEGIN MAPPING →</button>
  <div style="margin-top:16px;font-size:9px;color:#1e293b;letter-spacing:1px;">or</div>
  <a href="#" onclick="document.getElementById('intro').style.display='none';document.getElementById('pipeline-modal').classList.add('open');return false;" style="display:inline-block;margin-top:8px;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#ff3cac;text-decoration:none;border:1px solid rgba(255,60,172,0.25);padding:6px 18px;border-radius:20px;">🎬 explore the pipeline ↗</a>
</div>
...
function unlockEmpire(){
  ...
  persist();
  // Mark as unlocked so nvidia gate never fires again
  ...
  // Log completion server-side then reload so jar appears
  const fd = new FormData(); fd.append('id', store.id);
  fetch('/soul-complete.php', {method:'POST', body:fd})
    .then(()=>{ setTimeout(()=>location.reload(),2200); })
    .catch(()=>{ setTimeout(()=>location.reload(),2200); });
}
...
<!-- ══════════════════════════════════════════
     PIPELINE PILL BUTTON
══════════════════════════════════════════ -->
<div id="pipeline-pill" onclick="document.getElementById('pipeline-modal').classList.add('open')">
  <div class="pill-dot"></div>
  🎬 THE PIPELINE
</div>

<!-- ══════════════════════════════════════════
     PIPELINE MODAL — phone div
══════════════════════════════════════════ -->
<div id="pipeline-modal">
  <button id="pipeline-close" onclick="document.getElementById('pipeline-modal').classList.remove('open')">✕ CLOSE</button>

  <div class="pl-phone">
    <div class="pl-island"></div>
    <div class="pl-inner">

      <div class="pl-eyebrow">ShortFactory</div>
      <div class="pl-brand">ADVERTAINMENT</div>
      <div class="pl-sub">The Machine</div>

      <!-- 95% progress bar -->
      <div class="pl-pct-row">
        <div class="pl-pct-label">Pipeline completion</div>
        <div class="pl-pct-val">95%</div>
      </div>
      <div class="pl-bar">
        <div class="pl-bar-fill"></div>
        <div class="pl-bar-gap" title="Final 5% — open to contributors"></div>
      </div>

      <!-- What's in -->
      <div class="pl-check">
        <div class="pl-tick">✓</div>
        <div class="pl-check-text"><b>The Pipeline</b> — hook → tension → game → gate. Proven. Deployed. Live.</div>
      </div>
      <div class="pl-check">
        <div class="pl-tick">✓</div>
        <div class="pl-check-text"><b>The Vibe</b> — ADVERTainment. You perform behind the words. Invisible advertising.</div>
      </div>
      <div class="pl-check">
        <div class="pl-tick">✓</div>
        <div class="pl-check-text"><b>The Theory</b> — soul mapping, emotional physics, the full architecture. Filed. DOIs on record.</div>
      </div>
      <div class="pl-check">
        <div class="pl-tick">✓</div>
        <div class="pl-check-text"><b>The Creator Tools</b> — anyone can build + deploy their own danger game in minutes.</div>
      </div>

      <!-- The call -->
      <div class="pl-call">
        <div class="pl-call-title">The Final 5%</div>
        <div class="pl-call-body">
          That's on <b>you</b>.<br><br>
          <b>Advertisers</b> — bring your product to this water.<br>
          <b>Creators</b> — push ShortFactory to your audience.<br><br>
          I built the horse. I built the trough.<br>
          <b>Drinking is up to the horse.</b>
        </div>
      </div>

      <!-- CTAs -->
      <a href="/squid-advertainment.html" class="pl-cta-main">🎭 EXPERIENCE THE PIPELINE →</a>
      <a href="/dangergame-creator.html" class="pl-cta-sec">🎬 BUILD YOUR OWN GAME →</a>

      <div class="pl-quote">"I bring the horse to water.<br>Drinking is up to the horse."</div>

    </div>
  </div>
</div>

<script src="/admin-lock.js"></script>
</body>
</html>
