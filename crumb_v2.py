"""
CRUMB v2 — Phase-Aware Hybrid (28 Apr 2026)
=============================================
Built on top of v1 baseline. Adds a sparse "phase fingerprint":
the top-K spectral peaks per frame, with their bin indices and
quantized phases, so Griffin-Lim starts from a near-correct phase
instead of random.

WHY: v1's whispery character comes from random phase init in
Griffin-Lim. The cochlea (mel) tells us *what* frequencies are
present; the peak fingerprint tells us *where in the cycle* the
loudest ones are. The two together let the iteration lock to
pitched content instead of smearing.

WHAT GETS STORED (vs v1):
  v1:  zlib(header + 6-bit mel-dB)             → 186 KB
  v2:  zlib(header + 6-bit mel + peak block)   → ~210 KB (target)
       peak block = K=6 peaks/frame × (10-bit bin, 5-bit phase)

DECODER:
  unpack mel + peaks → linear mag (mel-pseudo-inv) → init STFT
  phase from peaks (random elsewhere) → Griffin-Lim 80 iters → ISTFT
...
def reconstruct_v2(mel_db, peaks_idx, peaks_phase, sr, n_fft, hop, n_mels):
    ...
    # 2. mel → linear magnitude
    mag = mel_to_linear(mel_db, sr, n_fft, n_mels)
    # 3. sharpen at peak bins (less aggressive — was 2.5)
    mag = sharpen_peaks(mag, peaks_idx, scale=2.0)
    # 4. surgical notch at 1 kHz (Dan-discovered propeller residual)
    mag = baked_eq(mag, sr, n_fft, freq=1000, gain_db=-3, q=3.0)
    # 5. seed-audio carrier from stored peaks + phase
    seed = carrier_from_peaks(peaks_idx, peaks_phase, mag, n_fft, hop)
    # 6. Griffin-Lim convergence
    audio = griffin_lim_seeded(mag, seed, n_fft, hop)
...
def main():
    ...
    print(f"[CRUMB v2] Encoding (mel + {N_PEAKS}-peak phase fingerprint)...")
    blob, mel_db = encode_v2(audio, sr)
    crumb_path = out_dir / (stem + '.crumb.v2')
    with open(crumb_path, 'wb') as f:
        f.write(blob)
    crumb_bytes = len(blob)
    print(f"           Crumb v2 binary: {crumb_bytes:,} bytes ({crumb_bytes/1024:.1f} KB)")
    print(f"           Compression vs source: {orig_bytes/crumb_bytes:.1f}x")
    print(f"           Compression vs PCM:    {pcm_bytes/crumb_bytes:.1f}x")
    print()

    print(f"[CRUMB v2] Decoding (phase-aware Griffin-Lim {GL_ITERS} iters)...")
    ...
    recon_path = out_dir / (stem + '.crumb.v2.wav')
    ...
    print()
    print("=" * 60)
    print("  CRUMB v2 — Phase-Aware Hybrid")
    print("=" * 60)
    print(f"  Source:        {orig_bytes:>12,} bytes  ({orig_bytes/1024:.1f} KB)")
    print(f"  Crumb v2:      {crumb_bytes:>12,} bytes  ({crumb_bytes/1024:.1f} KB)")
    ...
    print()
    print(f"  Listen: http://localhost:8888/crumb-player_v2.html")
    print()
