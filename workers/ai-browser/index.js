...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/screenshot' && request.method === 'POST') {
      ...
      try {
        await page.goto(body.url || 'https://www.shortfactory.shop', { waitUntil: 'networkidle0', timeout: 15000 });
    ...
    if (url.pathname === '/test-game' && request.method === 'POST') {
      ...
      try {
        await page.goto('https://www.shortfactory.shop/trump/game/', { waitUntil: 'networkidle0', timeout: 20000 });
    ...
    if (url.pathname === '/crawl' && request.method === 'POST') {
      ...
      const results = [];
      try {
        await page.goto(body.url || 'https://www.shortfactory.shop', { waitUntil: 'domcontentloaded', timeout: 15000 });
