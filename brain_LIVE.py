...
class CortexBrain:
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        return
                    time.sleep(0.05)
            tmp = str(self.brain_file) + f'.tmp.{os.getpid()}'
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # PATTERN MINER — surfaces implicit knowledge structures
    # ═══════════════════════════════════════════════════════════════════════════

    def mine_patterns(self, min_frequency=2, max_patterns=40):
        """
        Walk the rels graph to surface recurring knowledge structures.

        Two pattern types:
        1. CONVERGENT  — many words share the same rel→target
           e.g. [X] causes suffering (23 sources) reveals a cluster around suffering
        2. CAUSAL CHAIN — A causes B, B causes C → A causes C (transitivity)
           e.g. fear → causes → panic → causes → shutdown

        Stores results in data['patterns']. Called by feeds service.
        """
        nodes = self.data['nodes']

        # --- Convergent patterns: (rel_type, target) → [source words] ---
        convergent = {}
        for word, node in nodes.items():
            for rel_type, targets in node.get('rels', {}).items():
                if rel_type == 'not':
                    continue
                for target in (targets if isinstance(targets, list) else [targets]):
                    key = (rel_type, target)
                    convergent.setdefault(key, []).append(word)

        conv_patterns = []
        for (rel_type, target), sources in convergent.items():
            if len(sources) >= min_frequency:
                conv_patterns.append({
                    'type': 'convergent',
                    'pattern': f'[X] {rel_type} {target}',
                    'rel': rel_type,
                    'target': target,
                    'sources': sources[:12],
                    'frequency': len(sources),
                })
        conv_patterns.sort(key=lambda x: -x['frequency'])

        # --- Causal chains: A causes B causes C ---
        causes_map = {}
        for word, node in nodes.items():
            c = node.get('rels', {}).get('causes', [])
            if c:
                causes_map[word] = c if isinstance(c, list) else [c]

        chain_patterns = []
        seen = set()
        for word_a, b_list in causes_map.items():
            for word_b in b_list:
                if word_b not in causes_map:
                    continue
                for word_c in causes_map[word_b]:
                    if word_c == word_a:
                        continue
                    key = (word_a, word_b, word_c)
                    if key in seen:
                        continue
                    seen.add(key)
                    chain_patterns.append({
                        'type': 'causal_chain',
                        'pattern': f'{word_a} → {word_b} → {word_c}',
                        'chain': [word_a, word_b, word_c],
                        'rel': 'causes',
                        'frequency': 1,
                    })

        all_patterns = conv_patterns[:max_patterns // 2] + chain_patterns[:max_patterns // 2]

        self.data['patterns'] = all_patterns
        self.data.setdefault('stats', {})
        self.data['stats']['patterns_mined'] = len(all_patterns)
        self.data['stats']['patterns_last'] = time.strftime('%Y-%m-%d %H:%M:%S')

        self.save()
        print(f'[BRAIN] Patterns: {len(conv_patterns)} convergent, {len(chain_patterns)} causal chains')
        return all_patterns

    # ═══════════════════════════════════════════════════════════════════════════
    # ASSOCIATION CHAIN BUILDER — multi-hop rels traversal
    # ═══════════════════════════════════════════════════════════════════════════

    def build_chains(self, word, max_depth=3, rel_types=None):
        """
        Walk the rels graph from a seed word to discover indirect connections.

        Example: fear → [causes] → panic → [causes] → shutdown
                 god  → [is_a]   → creator → [has_a] → world

        Returns list of chain dicts, longest first.
        Caches top-5 chain texts on the node for fast retrieval.
        """
        if rel_types is None:
            rel_types = {'causes', 'is_a', 'has_a', 'used_for', 'part_of', 'synonym'}
                ...
                for target in targets:
                    ...
                    if len(new_path) >= 2:
                        chains.append({
                            ...
                            'path': new_path,
                            'length': len(new_path),
                            'text': word + ' ' + ' '.join(f'→[{r}]→ {t}' for r, t in new_path),
                        })
        ...
        # Cache on node
        nodes.get(word, {})['chains'] = [c['text'] for c in chains[:5]]

        return chains[:20]

    def get_patterns(self):
        """Return mined patterns, mining if not yet done."""
        return self.data.get('patterns') or self.mine_patterns()
