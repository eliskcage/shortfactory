...
# MemeQ keyword patterns for live matching (loaded from memeq_graph.json)
_MEMEQ_PATTERNS = {}
try:
...
def _match_memeq_patterns(text):
    """Return list of matching MemeQ node keys for the given text."""
    t = text.lower()
...
class OnlineHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # ═══════════════════════════════════════════
            ...
            # Priority organ override for high stress
            priority = snap.get('priority_organ', 'brain')
            if pain > 0.6:
                priority = 'cortisol'
            elif pleasure > 0.7 and activity > 0:
                priority = 'dopamine'
