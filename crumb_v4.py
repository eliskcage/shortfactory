"""
CRUMB v4 — The Japanese Garden (28 Apr 2026)
=============================================
Decoder-only evolution of v3. Same encoded `.crumb` format — the
encoder is unchanged. v4 changes the playback strategy entirely.

DAN'S IDEA: "if I could see the whole replay process u could run
several needles across the teeth at different offsets and make a
new genom as the original plays to make some new points that put
the melody back in after the helicopter has been removed. it's
like the sand in a Japanese garden, we patched up the big line,
but now we need to rake some of the patterns back in ontop!"

THE TWO CHEATS, STACKED:

  1. MULTI-PASS DITHER ("dilute the gremlin"):
     decode the same crumb N=4 times with varied decoder
     parameters (smear kernel, sharpening factor, peak-stabilize
     window, GL iterations). Each pass has its helicopter at a
     slightly different time-frequency position; the music is
     invariant. Then median-combine the magnitude spectrograms
     across passes — the artifact components, which differ
     pass-to-pass, get suppressed by the median; the music, which
     is consistent across passes, survives.

  2. RAKE-BACK ("put the melody back on top of the smoothed sand"):
     compute the variance across passes per time-frequency cell.
     Low variance = music (consistent). High variance = artifact
     (varies with parameters). Build a confidence mask. Use it to
     selectively restore detail in the cells that all passes agree
     on, rejecting cells where they disagree.

ENCODED FILE FORMAT: identical to v2/v3. Pure decoder change.

COST: 4× decode time vs v3 (~2 minutes for 136s on CPU).
For voice biscuits this is encode-once-decode-many, so amortized fine.
For real-time use, drop to N=2 passes or use neural vocoder (v5+).

Usage: python crumb_v4.py <audiofile.mp3|wav>
"""
...
def _single_pass(mel_db, peaks_idx, peaks_phase, sr, n_fft, hop, n_mels,
                 smear_k, sharpen_scale, stabilize_w, gl_iters, mag_dither):
    """One decode pass with given parameters. Same hop/upsample for all
    passes so STFTs align for cross-pass median later."""
    p_idx = stabilize_peaks(peaks_idx, window=stabilize_w)
    upsample = 2
    fine_hop = hop // upsample
    p_idx_up, p_phase_up = time_upsample_peaks(p_idx, peaks_phase, upsample)

    mag = mel_to_linear(mel_db, sr, n_fft, n_mels)
    mag = time_upsample_mag(mag, upsample)
    mag = smear_mag_time(mag, kernel_frames=smear_k)
    if mag_dither > 0:
        rng = np.random.default_rng(int(mag_dither * 1e6))
        noise = 1.0 + (rng.random(mag.shape).astype(np.float32) - 0.5) * mag_dither
        mag = mag * noise
    mag = sharpen_peaks(mag, p_idx_up, scale=sharpen_scale)

    seed = carrier_from_peaks(p_idx_up, p_phase_up, mag, n_fft, fine_hop)
    audio = griffin_lim_seeded(mag, seed, n_fft, fine_hop, n_iter=gl_iters)
    return audio


def reconstruct_v2(mel_db, peaks_idx, peaks_phase, sr, n_fft, hop, n_mels):
    """v4 — Japanese Garden. Multi-pass dither + rake-back."""

    # ── Cheat 1: dilute the gremlin (4 passes with varied parameters) ──
    PASS_CONFIGS = [
        # (smear_k, sharpen, stabilize_w, gl_iters, mag_dither)
        (5, 2.0, 3, 60, 0.00),
        (7, 1.7, 3, 50, 0.02),
        (3, 2.3, 5, 70, 0.00),
        (9, 1.5, 1, 50, 0.03),
    ]
    print(f"[v4] running {len(PASS_CONFIGS)} decode passes...")
    audios = []
    for i, cfg in enumerate(PASS_CONFIGS):
        print(f"     pass {i+1}/{len(PASS_CONFIGS)}: smear={cfg[0]} sharp={cfg[1]} stab={cfg[2]} gl={cfg[3]} dither={cfg[4]}")
        a = _single_pass(mel_db, peaks_idx, peaks_phase, sr, n_fft, hop, n_mels, *cfg)
        audios.append(a)

    # align lengths
    n_min = min(len(a) for a in audios)
    audios = [a[:n_min] for a in audios]

    fine_hop = hop // 2
    print(f"[v4] median-combining magnitude spectrograms across passes...")
    specs = [stft(a, n_fft, fine_hop) for a in audios]
    n_frames_min = min(s.shape[1] for s in specs)
    specs = [s[:, :n_frames_min] for s in specs]
    mag_stack = np.stack([np.abs(s) for s in specs], axis=0)

    mag_median = np.median(mag_stack, axis=0).astype(np.float32)

    # ── Cheat 2: rake the patterns back on top of the smoothed sand ──
    print(f"[v4] computing music-confidence mask from cross-pass variance...")
    mag_std = np.std(mag_stack, axis=0).astype(np.float32)
    confidence = mag_median / (mag_std + 0.5 * mag_median + 1e-10)
    confidence = np.clip(confidence, 0.0, 1.0)

    mag_max = np.max(mag_stack, axis=0).astype(np.float32)
    mag_raked = (1.0 - confidence) * mag_median + confidence * mag_max

    phase_ref = np.angle(specs[0])
    spec_final = (mag_raked * np.exp(1j * phase_ref)).astype(np.complex64)
    audio = istft(spec_final, fine_hop, n_fft)

    # ── Cipher EQ — bake Dan's playback preset into the decoder ──
    print(f"[v4] applying baked Cipher EQ (1k/3k/6k -12dB)...")
    spec_a = stft(audio, n_fft, fine_hop)
    bin_freqs = np.fft.rfftfreq(n_fft, 1.0 / sr)
    eq_curve = np.ones(len(bin_freqs), dtype=np.float32)
    for f0 in (1000.0, 3000.0, 6000.0):
        bw = f0 / 3.0
        weight = np.exp(-((bin_freqs - f0) / (bw / 2.0)) ** 2)
        eq_curve *= 1.0 + (10**(-12/20) - 1.0) * weight
    spec_a *= eq_curve[:, None]
    audio = istft(spec_a, fine_hop, n_fft)
...
def main():
    ...
    print(f"[CRUMB v4] Encoding (same as v2: mel + {N_PEAKS}-peak phase fingerprint)...")
    blob, mel_db = encode_v2(audio, sr)
    crumb_path = out_dir / (stem + '.crumb.v4')
    ...
    print(f"[CRUMB v4] Decoding (smear-the-teeth: 2x time-upsample, 5-frame mag smear, hop/2)...")
    ...
    recon_path = out_dir / (stem + '.crumb.v4.wav')
    ...
    print()
    print("=" * 60)
    print("  CRUMB v4 — Japanese Garden")
    print("=" * 60)
