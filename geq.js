...
/* ── VERTEX EQUILIBRIUM ── */
// [0]=PAST(210°), [1]=PRESENT(90°), [2]=FUTURE(330°)
let eqVerts = [{x:0,y:0},{x:0,y:0},{x:0,y:0}];
let vertices = [{x:0,y:0},{x:0,y:0},{x:0,y:0}];
let inverted  = [false, false, false];
...
let globalInverted = false;
let inversionFlash = 0;
let warningActive  = false;
let warningEm      = null;
let autoInverting  = false;
let autoInvertT    = 0;
let lastTs=0;
...
let emotionTargetVerts = [{x:0,y:0},{x:0,y:0},{x:0,y:0}];
...
function onDown(e){
  ...
  for(let i=0;i<3;i++){
    const v=vertices[i];
    if(Math.hypot(p.x-v.x, p.y-v.y)<28){
...
function onMove(e){
  ...
  if(prevDot >= 0 && newDot < 0) triggerInversion(i);
...
function checkHoverCursor(e){
  ...
  let near=false;
  for(let i=0;i<3;i++){
    if(Math.hypot(p.x-vertices[i].x, p.y-vertices[i].y)<28){near=true;break;}
  }
...
function updateEmotionCycle(dt){
  ...
  if(phase < DWELL){
    ...
    const morphFrac = easeIO((phase-DWELL)/MORPH);
...
function drawEmotionTriangle(T){
  ...
  grd.addColorStop(0,`rgba(${cr},${cg},${cb},${(0.13*breath).toFixed(3)})`);
  grd.addColorStop(1,'rgba(0,0,0,0)');
  ctx.fillStyle=grd; ctx.fill();
  ...
  tv.forEach((v,i)=>i===0?ctx.moveTo(v.x,v.y):ctx.lineTo(v.x,v.y));
  ctx.closePath();
  ctx.strokeStyle=`rgba(${cr},${cg},${cb},${(0.7*breath).toFixed(3)})`;
...
function drawVertices(T){
  vertices.forEach((v,i)=>{
    ...
    ctx.strokeStyle=inv
      ? `rgba(220,38,38,${(0.1+0.05*Math.sin(T+i)).toFixed(3)})`
      : `rgba(255,255,255,${(0.1+0.05*Math.sin(T+i)).toFixed(3)})`;
...
/* ── INIT (after everything declared) ── */
resize();
